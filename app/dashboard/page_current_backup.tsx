...
function RevCalc({onAddPricing}:{onAddPricing:()=>void}) {
  ...
  return <div style={{border:'1px solid rgba(255,106,0,.15)',padding:'14px',background:'rgba(255,106,0,.03)',borderLeft:'2px solid rgba(255,106,0,.4)',opacity:show?1:0,transform:show?'translateY(0)':'translateY(8px)',transition:'all .5s ease',animation:'fu .4s ease'}}>
    <div className="flex items-center gap-1.5 mb-2"><span style={{fontSize:10,color:'rgba(255,106,0,.7)'}}>◈</span><span style={{fontFamily:"'Orbitron',sans-serif",fontSize:8,letterSpacing:'.25em',color:'rgba(255,106,0,.5)'}}>REVENUE CALCULATOR</span></div>
    ...
    <div className="flex gap-2 mb-2">
      <div className="flex-1">
        <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:7,letterSpacing:'.15em',color:'rgba(255,106,0,.4)',marginBottom:4}}>PRICE / MONTH</div>
        <div className="flex items-center" style={{border:'1px solid rgba(255,106,0,.15)',background:'rgba(255,106,0,.04)'}}>
          <span style={{padding:'6px 8px',fontSize:12,color:'rgba(255,106,0,.5)',borderRight:'1px solid rgba(255,106,0,.1)'}}>$</span>
      ...
      <div className="flex-1">
        <div style={{fontFamily:"'Orbitron',sans-serif",fontSize:7,letterSpacing:'.15em',color:'rgba(255,106,0,.4)',marginBottom:4}}>CUSTOMERS</div>
        <div className="flex items-center" style={{border:'1px solid rgba(255,106,0,.15)',background:'rgba(255,106,0,.04)'}}>
          <span style={{padding:'6px 8px',fontSize:12,color:'rgba(255,106,0,.5)',borderRight:'1px solid rgba(255,106,0,.1)'}}>×</span>
    ...
    {p>0&&c>0&&<div style={{padding:'10px 12px',background:'rgba(255,106,0,.04)',border:'1px solid rgba(255,106,0,.1)',marginTop:8}}>
      <div className="flex justify-between mb-1"><span style={{fontSize:11,color:'rgba(195,200,215,.6)'}}>Monthly</span><span style={{fontSize:14,color:'#FF6A00',fontWeight:700}}>${monthly.toLocaleString()}</span></div>
      <div className="flex justify-between"><span style={{fontSize:11,color:'rgba(195,200,215,.6)'}}>Yearly</span><span style={{fontSize:14,color:'#FF6A00',fontWeight:700}}>${yearly.toLocaleString()}</span></div>
      <div style={{marginTop:8,paddingTop:8,borderTop:'1px solid rgba(255,106,0,.07)'}}>
        <span className="cursor-pointer" onClick={onAddPricing} style={{fontSize:10,color:'rgba(255,106,0,.7)',padding:'4px 10px',border:'1px solid rgba(255,106,0,.2)',background:'rgba(255,106,0,.04)'}}>Want me to add a pricing page? →</span>
      </div>
    </div>}
  </div>
}
...
export default function DashboardPage() {
  ...
  const [panelTab,setPanelTab]=useState<'coach'|'journal'|'learn'>('coach')
  ...
  useEffect(()=>{
    ...
    const cleanCode=generatedCode.replace(/^import\s.*?[\r\n]+/gm,'').replace('export default function','function').replace('export default ','')
  ...
  const handleBuild=useCallback(async()=>{
    ...
    if(wordCount<30){
      setMsgs([{role:'coach',text:'Short prompt detected. Let me sharpen your vision...'},{role:'user',text:prompt}])
      ...
      try{
        ...
        if(enhData.success&&enhData.enhanced){buildPrompt=enhData.enhanced;setMsgs(prev=>[...prev,{role:'coach',text:'Here\'s what I\'m building: '+enhData.enhanced},{role:'coach',text:'Building now \u2014 I\'ll walk you through each step.',type:'building'}])}
        else{setMsgs(prev=>[...prev,{role:'coach',text:'Building now \u2014 I\'ll walk you through each step.',type:'building'}])}
      }catch{setMsgs(prev=>[...prev,{role:'coach',text:'Building now \u2014 I\'ll walk you through each step.',type:'building'}])}
    }else{
    ...
    try{
      ...
      if(data.success){
        ...
        setMsgs(prev=>[...prev,{role:'coach',text:data.narration||'Your app is live. Here\'s what I\'d suggest next:',type:'summary'}])
  ...
  const handleRefine=useCallback(async()=>{
    ...
    setMsgs(prev=>[...prev,{role:'user',text:refineText},{role:'coach',text:'Refining your app now...',type:'building'}])
    setRefineText('')
    let step=0;setNarrText(STEPS[0][0]);setNarrTeach(STEPS[0][1])
    const ni=setInterval(()=>{step++;if(step>=STEPS.length){clearInterval(ni);return};setNarrText(STEPS[step][0]);setNarrTeach(STEPS[step][1])},800)
    try{
      ...
      if(data.success){
        ...
        if(data.narration)setMsgs(prev=>[...prev,{role:'coach',text:data.narration,type:'summary'}])
      }else{setAppState('complete');setShowNarr(false);setMsgs(prev=>[...prev,{role:'coach',text:data.message||'Couldn\'t refine. Try again.'}])}
    }catch{clearInterval(ni);setAppState('complete');setShowNarr(false);setMsgs(prev=>[...prev,{role:'coach',text:'The Grid encountered an error. Try again.'}])}
  ...
  const loadApp=(app:{id:string,name:string,code:string})=>{
    ...
    setGeneratedCode(app.code.replace(/^import\s.*?[\r\n]+/gm,'').replace('export default function','function').replace('export default ',''))
    ...
    setMsgs([{role:'coach',text:'Loaded '+app.name+'. What would you like to change?',type:'summary'}])
  }
  ...
  return <main className="relative min-h-screen overflow-hidden">
    ...
    <div className="fixed left-0 top-0 bottom-0 z-50 flex flex-col transition-all duration-300" style={{width:sbW,background:'rgba(4,6,14,.96)',borderRight:'1px solid rgba(0,229,255,.07)',backdropFilter:'blur(18px)',opacity:entered?1:0,transition:'opacity .5s ease'}}>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2.5">
        ...
        {!sbCol&&<div style={{fontFamily:UI,fontSize:8,letterSpacing:'.2em',color:'rgba(0,229,255,.4)',padding:'10px 10px 4px'}}>PROJECTS ({savedApps.length})</div>}
        {!sbCol&&savedApps.map(app=><div key={app.id} className="flex items-center gap-2.5 cursor-pointer mb-px" onClick={()=>loadApp(app)} style={{padding:'8px 10px',fontSize:12,color:appId===app.id?'#00E5FF':'rgba(195,200,215,.75)',background:appId===app.id?'rgba(0,229,255,.04)':'transparent',border:`1px solid ${appId===app.id?'rgba(0,229,255,.15)':'transparent'}`}}><span style={{fontSize:14,width:20,textAlign:'center'}}>◫</span><span className="flex-1 truncate">{app.name}</span></div>)}
        {!sbCol&&savedApps.length===0&&<div style={{padding:'8px 10px',fontSize:11,color:'rgba(115,122,142,.35)'}}>No projects yet</div>}
          ...
          <div className="flex flex-col h-full" style={{width:370,minWidth:370,background:'rgba(4,6,14,.96)',borderRight:'1px solid rgba(0,229,255,.07)',backdropFilter:'blur(18px)'}}>
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
              {msgs.map((m,i)=><Msg key={i} role={m.role} text={m.text}>
                ...
                {m.type==='summary'&&suggestions.length>0&&<div className="mt-3 flex flex-wrap gap-1.5">{suggestions.map((s,j)=><Sug key={j} text={s}/>)}</div>}
              </Msg>)}
