...
export default function DashboardPage() {
  ...
  const handleBuild=useCallback(async()=>{
    ...
    let buildPrompt=prompt;let capturedBlueprintId:string|null=null;let capturedPhotoQuery:string|null=null
    try{const eRes=await fetch('/api/enhance',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt})});const eData=await eRes.json();if(eData.success&&eData.enhanced){buildPrompt=eData.enhanced;capturedBlueprintId=eData.blueprintId||null;capturedPhotoQuery=eData.photoQuery||null;setPromptAnalysis({original:prompt,enhanced:eData.enhanced,layers:eData.layers||[],score:eData.promptScore||3,blueprintId:capturedBlueprintId});setMsgs(prev=>[...prev.slice(0,-1),{role:'cipher',text:'Prompt strengthened. Building now.',type:'building'}])}}catch(e){}
    ...
    try{
      const res=await fetch('/api/build',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:buildPrompt,persona:'operator',appId:null,blueprintId:capturedBlueprintId||null,photoQuery:capturedPhotoQuery||null,intakeAnswers:intakeAnswers||null})})
      ...
      if(data.success){
        ...
        setTimeout(()=>setShowStrip(true),400);setTimeout(()=>setShowModes(true),1000);setTimeout(()=>{const sb=createClient();sb.auth.getUser().then(({data:{user:u}})=>{if(u)sb.from('apps').select('id,name,code,updated_at').eq('user_id',u.id).order('updated_at',{ascending:false}).limit(50).then(({data:a})=>{if(a)setSavedApps(a)})})},2000)
      }else{
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {appState==='idle'&&<div className="absolute inset-0 flex items-center justify-center px-6 z-20"><div className="flex flex-col items-center max-w-xl w-full text-center">
          ...
          <div className="w-full max-w-lg" style={{background:'rgba(4,6,14,.96)',border:'1px solid rgba(0,229,255,.15)',borderBottom:'2px solid rgba(0,229,255,.3)',padding:16,backdropFilter:'blur(18px)'}}>
            <div style={{position:'relative'}}>{showIntake&&<div style={{position:'absolute',inset:0,zIndex:50,background:'#0B0D14',border:'1px solid rgba(0,229,255,.15)'}}><IntakeFlow onComplete={(p,a)=>{setShowIntake(false);setIntakeAnswers(a);setPrompt(p);setTimeout(()=>handleBuild(),200)}} onSkip={()=>setShowIntake(false)}/></div>}<canvas ref={buildCanvasRef} style={{position:'absolute',inset:0,pointerEvents:'none',zIndex:1}}/><textarea value={prompt} onChange={e=>{setPrompt(e.target.value);const ls=e.target.value.split('\n');const la=ls[ls.length-1];const cx=Math.min(14+la.length*8.5,480);const cy=Math.min(14+(ls.length-1)*24+12,50);for(let j=0;j<2;j++)buildParticlesRef.current.push({x:cx+(Math.random()-.5)*14,y:cy,vx:(Math.random()-.5)*.3,vy:-(0.3+Math.random()*0.6),life:1,decay:0.006+Math.random()*0.004,size:0.8+Math.random()*1.5})}} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&appState==='idle'&&prompt.trim()){e.preventDefault();handleBuild()}}} placeholder={PLACEHOLDERS[phIdx]} className="w-full bg-transparent outline-none resize-none" style={{color:'#F0F0FF',fontSize:17,height:60,lineHeight:'1.55',position:'relative',zIndex:2,background:'transparent'}}/><button onClick={()=>setShowIntake(true)} style={{position:'absolute',bottom:-28,left:0,display:'flex',alignItems:'center',gap:6,fontFamily:'"JetBrains Mono",monospace',fontSize:8,letterSpacing:'.12em',color:'rgba(0,229,255,.45)',background:'transparent',border:'none',padding:'4px 0',cursor:'pointer',transition:'all .2s',zIndex:3}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.color='#00E5FF'}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.color='rgba(0,229,255,.45)'}}><span style={{width:6,height:6,borderRadius:'50%',background:'#00E5FF',flexShrink:0,display:'inline-block',animation:'briefPulse 2s ease infinite'}}/> Let Cipher guide your brief</button></div>
        ...
        {appState!=='idle'&&<div className="flex w-full h-full" style={{animation:'fu .5s ease'}}>
          <div className="flex flex-col h-full" style={{width:370,minWidth:370,background:'rgba(4,6,14,.96)',borderRight:'1px solid rgba(0,229,255,.07)',backdropFilter:'blur(18px)'}}>
            ...
            <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3">
              ...
              {showIntake&&<div style={{position:'fixed',top:0,left:0,right:0,bottom:0,zIndex:999,background:'rgba(11,13,20,.97)',backdropFilter:'blur(8px)',display:'flex',alignItems:'stretch'}}><div style={{width:'100%',maxWidth:480,margin:'auto',height:'80vh',border:'1px solid rgba(0,229,255,.1)',background:'#0B0D14',display:'flex',flexDirection:'column'}}><IntakeFlow onComplete={(p,a)=>{setShowIntake(false);setIntakeAnswers(a);setPrompt(p);setTimeout(()=>{const btn=document.getElementById('buildTrigger');if(btn)(btn as HTMLButtonElement).click()},150)}} onSkip={()=>setShowIntake(false)}/></div></div>}
            ...
            <div className="px-3 pb-3 flex-shrink-0" style={{background:'rgba(8,11,22,.93)',borderTop:'1px solid rgba(0,229,255,.07)'}}>
              ...
              :appState==='idle'&&showIntake?<div style={{height:340,position:'relative',border:'1px solid rgba(0,229,255,.1)'}}><IntakeFlow onComplete={(p,a)=>{setShowIntake(false);setIntakeAnswers(a);setRefineText(p);setTimeout(()=>{const btn=document.getElementById('buildTrigger');if(btn)(btn as HTMLButtonElement).click()},100)}} onSkip={()=>setShowIntake(false)}/></div>:appState==='idle'?<div style={{position:'relative'}}><button onClick={()=>setShowIntake(true)} style={{display:'flex',alignItems:'center',gap:6,fontFamily:'"JetBrains Mono",monospace',fontSize:8,letterSpacing:'.12em',color:'rgba(0,229,255,.5)',background:'rgba(0,229,255,.04)',border:'1px solid rgba(0,229,255,.12)',padding:'4px 12px',cursor:'pointer',transition:'all .2s',marginBottom:6,width:'100%'}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.color='#00E5FF';(e.currentTarget as HTMLElement).style.borderColor='rgba(0,229,255,.3)'}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.color='rgba(0,229,255,.5)';(e.currentTarget as HTMLElement).style.borderColor='rgba(0,229,255,.12)'}}><span style={{width:6,height:6,borderRadius:'50%',background:'#00E5FF',flexShrink:0,display:'inline-block'}}/> Let Cipher guide your brief</button><textarea className="w-full bg-transparent outline-none resize-none" placeholder="Or type your prompt directly..." value={refineText} onChange={e=>setRefineText(e.target.value)} style={{background:'rgba(8,11,22,.7)',border:'1px solid rgba(0,229,255,.07)',borderBottom:'2px solid rgba(0,229,255,.15)',color:'#F0F0FF',fontSize:15,padding:'12px 14px',height:56,lineHeight:'1.5'}}/></div>:<textarea className="w-full bg-transparent outline-none resize-none" placeholder="What would make this better?" value={refineText} onChange={e=>setRefineText(e.target.value)} style={{background:'rgba(8,11,22,.7)',border:'1px solid rgba(0,229,255,.07)',borderBottom:'2px solid rgba(0,229,255,.15)',color:'#F0F0FF',fontSize:15,padding:'12px 14px',height:56,lineHeight:'1.5'}}/>}
              <div className="flex items-center justify-between mt-2"><div className="flex gap-1 items-center">{['\ud83d\udcce','\ud83c\udfa4','\ud83d\udcf7','\u25c7'].map(i=><div key={i} className="flex items-center justify-center cursor-pointer" style={{width:22,height:22,border:'1px solid rgba(0,229,255,.07)',color:'rgba(195,200,215,.55)',fontSize:10}}>{i}</div>)}</div><button onClick={async()=>{if(!refineText.trim())return;setAppState('building');setShowNarr(true);setNarrText('Refining your app...');setNarrTeach('applying your changes');setMsgs(prev=>[...prev,{role:'user',text:refineText},{role:'cipher',text:'On it. Applying your changes now.',type:'building'}]);const res=await fetch('/api/build',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt:refineText+' \n\nPrevious code to modify: '+generatedCode.slice(0,2000),persona:'operator',appId:appId})});const data=await res.json();if(data.success){setGeneratedCode((data.code||'').replace('export default function','function').replace('export default ','').replace('const App = () =>','function App()').replace('const App = ()=>','function App()').replace('const App = () =>{','function App(){'));setSuggestions(data.suggestions||[]);if(data.appId)setAppId(data.appId);setBuildScore(data.score?Number(data.score):Math.floor(40+Math.random()*20));if(data.tokenUsage)setTokenUsage(data.tokenUsage);setAppState('complete');setShowNarr(false);setRefineText('');setMsgs(prev=>[...prev,{role:'cipher',text:data.narration||'Changes applied.',type:'summary'},{role:'cipher',text:'Cipher is here — what needs work?',type:'suggestion'}])}else{setAppState('complete');setShowNarr(false);setMsgs(prev=>[...prev,{role:'cipher',text:data.message||'Something went wrong. Try again.'}])}}} style={{fontFamily:UI,fontSize:9,fontWeight:700,letterSpacing:'.16em',color:'#000308',background:'#00E5FF',border:'none',padding:'6px 14px',cursor:'pointer'}} id='buildTrigger'>{activeMode==='PLAN'?'SAVE NOTE':'SEND →'}</button>{activeMode==='BUILD'&&appState==='idle'&&!showIntake&&<button onClick={()=>setShowIntake(true)} style={{fontFamily:UI,fontSize:8,letterSpacing:'.1em',color:'rgba(0,229,255,.6)',background:'rgba(0,229,255,.04)',border:'1px solid rgba(0,229,255,.15)',padding:'6px 10px',cursor:'pointer',transition:'all .2s',marginLeft:4}} title='Let Cipher guide your brief'>✶ Brief</button>}</div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {appState==='complete'&&<div style={{padding:'12px 16px',borderBottom:'1px solid rgba(0,229,255,.05)'}}>
            ...
            <div className="flex gap-2">
              ...
              {appId&&<span className="cursor-pointer" onClick={()=>window.location.href='/dashboard/deploy/'+appId} style={{fontSize:10,fontWeight:600,color:'#3DD68C',padding:'4px 8px',border:'1px solid rgba(61,214,140,.3)',background:'rgba(61,214,140,.06)',letterSpacing:'.04em'}}>Take it live →</span>}
