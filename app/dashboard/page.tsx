  ...
  return <main className="relative min-h-screen overflow-hidden">
    ...
    <div className="fixed left-0 top-0 bottom-0 z-50 flex flex-col transition-all duration-300" style={{width:sbW,background:'rgba(4,6,14,.96)',borderRight:'1px solid rgba(0,229,255,.07)',backdropFilter:'blur(18px)',opacity:entered?1:0,transition:'opacity .5s ease'}}>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2.5">
        ...
        {[{k:'portal',l:'Client Portal'},{k:'saas',l:'SaaS Dashboard'},{k:'booking',l:'Booking System'},{k:'store',l:'Online Store'}].map(s=><div key={s.k} className="flex items-center gap-2.5 cursor-pointer mb-px" onClick={()=>{if(!sbCol)loadTpl(s.k)}} title={sbCol?s.l:undefined} style={{padding:sbCol?0:'8px 10px',width:sbCol?44:undefined,height:sbCol?44:undefined,margin:sbCol?'0 auto 3px':undefined,justifyContent:sbCol?'center':undefined,display:'flex',flexDirection:sbCol?'column' as any:undefined,alignItems:sbCol?'center':undefined,gap:sbCol?2:undefined,fontSize:13,color:'rgba(195,200,215,.8)',borderRadius:sbCol?8:undefined,border:'1px solid transparent'}}><span style={{fontSize:14,width:20,textAlign:'center'}}>◈</span>{sbCol&&<span style={{fontSize:6,color:'rgba(195,200,215,.35)',letterSpacing:'.02em',textAlign:'center',lineHeight:1}}>{s.l.split(' ')[0]}</span>}{!sbCol&&<span>{s.l}</span>}</div>)}
