  ...
  return <main className="relative min-h-screen overflow-hidden">
    ...
    <div className="fixed left-0 top-0 bottom-0 z-50 flex flex-col transition-all duration-300" style={{width:sbW,background:'rgba(4,6,14,.96)',borderRight:'1px solid rgba(0,229,255,.07)',backdropFilter:'blur(18px)',opacity:entered?1:0,transition:'opacity .5s ease'}}>
      ...
      {!sbCol&&<div className="flex flex-col gap-1 p-2" style={{borderTop:'1px solid rgba(0,229,255,.035)'}}>
        ...
        <div className="flex items-center gap-2 px-2.5 py-2"><div className="flex items-center justify-center" style={{width:28,height:28,border:'1px solid rgba(0,229,255,.15)',fontFamily:UI,fontSize:8,color:'#00E5FF'}}>{userName.slice(0,2).toUpperCase()}</div><div><div style={{fontSize:11,color:'rgba(195,200,215,.82)'}}>{userName}</div><div style={{fontFamily:UI,fontSize:8,color:'rgba(115,122,142,.35)',letterSpacing:'.1em'}}>OPERATOR</div></div></div>
        <div className="flex items-center justify-center cursor-pointer py-1.5 mt-1" onClick={async()=>{const sb=createClient();await sb.auth.signOut();window.location.href='/auth'}} style={{fontSize:9,fontFamily:UI,letterSpacing:'.15em',color:'rgba(255,49,49,.5)',border:'1px solid rgba(255,49,49,.1)',transition:'all .3s'}} onMouseEnter={e=>{e.currentTarget.style.color='#FF3131';e.currentTarget.style.borderColor='rgba(255,49,49,.3)'}} onMouseLeave={e=>{e.currentTarget.style.color='rgba(255,49,49,.5)';e.currentTarget.style.borderColor='rgba(255,49,49,.1)'}}>EXIT</div>
      </div>}
    </div>
