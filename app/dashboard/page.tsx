        ...
        {appState!=='idle'&&<div className="flex w-full h-full" style={{animation:'fu .5s ease'}}>
          <div className="flex flex-col h-full" style={{width:370,minWidth:370,background:'rgba(4,6,14,.96)',borderRight:'1px solid rgba(0,229,255,.07)',backdropFilter:'blur(18px)'}}>
            ...
            <div className="px-3 pb-3 flex-shrink-0" style={{background:'rgba(8,11,22,.93)',borderTop:'1px solid rgba(0,229,255,.07)'}}>
              ...
              :appState==='idle'&&showIntake?<div style={{height:340,position:'relative',border:'1px solid rgba(0,229,255,.1)'}}><IntakeFlow onComplete={(p,a)=>{setShowIntake(false);setIntakeAnswers(a);setRefineText(p);setTimeout(()=>{const btn=document.getElementById('buildTrigger');if(btn)(btn as HTMLButtonElement).click()},100)}} onSkip={()=>setShowIntake(false)}/></div>:appState==='idle'?<div style={{position:'relative'}}><textarea className="w-full bg-transparent outline-none resize-none" placeholder="Describe what you want to build..." value={refineText} onChange={e=>setRefineText(e.target.value)} style={{background:'rgba(8,11,22,.7)',border:'1px solid rgba(0,229,255,.07)',borderBottom:'2px solid rgba(0,229,255,.15)',color:'#F0F0FF',fontSize:15,padding:'12px 14px',height:56,lineHeight:'1.5'}}/><button onClick={()=>setShowIntake(true)} style={{position:'absolute',bottom:8,left:12,display:'flex',alignItems:'center',gap:6,fontFamily:'"JetBrains Mono",monospace',fontSize:8,letterSpacing:'.12em',color:'rgba(0,229,255,.5)',background:'rgba(0,229,255,.04)',border:'1px solid rgba(0,229,255,.12)',padding:'3px 10px',cursor:'pointer',transition:'all .2s'}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.color='#00E5FF';(e.currentTarget as HTMLElement).style.borderColor='rgba(0,229,255,.3)'}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.color='rgba(0,229,255,.5)';(e.currentTarget as HTMLElement).style.borderColor='rgba(0,229,255,.12)'}}><span style={{width:6,height:6,borderRadius:'50%',background:'#00E5FF',boxShadow:'0 0 0 0 rgba(0,229,255,.4)',animation:'briefPulse 2s ease infinite',display:'inline-block',flexShrink:0}}/> Let Cipher guide your brief</button></div>:<textarea className="w-full bg-transparent outline-none resize-none" placeholder="What would make this better?" value={refineText} onChange={e=>setRefineText(e.target.value)} style={{background:'rgba(8,11,22,.7)',border:'1px solid rgba(0,229,255,.07)',borderBottom:'2px solid rgba(0,229,255,.15)',color:'#F0F0FF',fontSize:15,padding:'12px 14px',height:56,lineHeight:'1.5'}}/>}
